...
const ToastContainer: React.FC<{ toasts: Toast[]; removeToast: (id: string) => void; dismissing: Set<string>; pauseToast: (id: string) => void; resumeToast: (id: string) => void }> = ({
  ...
  return (
    <div className="fixed top-4 right-4 left-4 md:left-auto z-[100] flex flex-col gap-3 max-w-sm">
      {toasts.map((toast) => (
        <div
          key={toast.id}
        ...
        >
          ...
          <div className="flex items-start gap-3 p-4">
            ...
            <button
              ...
              aria-label="关闭通知"
              className="flex-shrink-0 text-slate-500 hover:text-white hover:scale-110 active:scale-95 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-dark rounded-md"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
